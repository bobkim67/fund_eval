...
export function isScorable(f: Fund, p: Period): boolean {
  ...
  return (
    ...
    yd !== null && !isNaN(yd) &&
    f.aum !== null && f.aum > 0 &&
    f.fam_aek !== null && f.fam_aek > 0
  );
}

// 사용자 필터 (정상/온라인/퇴직연금은 서버 단에서 이미 적용됨)
export function passUserFilter(f: Fund, fs: FilterState): boolean {
  ...
  if (f.fam_aek === null || f.fam_aek < fs.fam_aek_min_oku * 1e8) return false;
...
// 운용사 점수 동적 계산:
//   각 펀드 → (sector_group, amc_nm) 키 → 그 그룹의 family AEK 합 → ranking → 1~100점
//   sector="전체" 일 때는 sector_group 무시하고 amc 별 family AEK 합 → ranking
export function computeAmcScores(
  ...
  aumBySectorAmc: Map<string, number>;     // "sector|amc" → family AEK 합
  aumByAmcOverall: Map<string, number>;    // amc → family AEK 합 (전체)
} {
  // sector × amc family AEK 합
  const sectorAmcAum = new Map<string, number>();
  const sectorAmcSet = new Map<string, Set<string>>(); // sector → amc set
  for (const f of funds) {
    if (!f.amc_nm || !f.fam_aek) continue;
    const key = `${f.sector_group}|${f.amc_nm}`;
    sectorAmcAum.set(key, (sectorAmcAum.get(key) ?? 0) + f.fam_aek);
  ...
  for (const f of funds) {
    if (!f.amc_nm || !f.fam_aek) continue;
    amcOverallAum.set(f.amc_nm, (amcOverallAum.get(f.amc_nm) ?? 0) + f.fam_aek);
  }
...
export function scoreFunds(
  ...
  const aumScores = rankScore(scorableFunds.map((f) => f.fam_aek));
  ...
  // 동적 운용사 점수 (sector_group 기준 또는 overall)
  ...
  scorableFunds.forEach((f, i) => {
    const sAum = aumScores[i];
    const sYld = yldScores[i];
    const sSharp = sharpScores[i];
    const sAmc = amcScores[i];
    const allOk = [sAum, sYld, sSharp, sAmc].every((s) => s !== null);
    const total = allOk
      ? weights.aum * (sAum as number) +
  ...
  return funds.map((f) => scoredMap.get(f.fund_cd) ?? {
    ...f,
    score_aum: null, score_yield_2y: null, score_sharp_2y: null,
